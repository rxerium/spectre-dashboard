import { Lock } from 'lucide-react';
import ScanResultPage from '../components/ScanResultPage';
import spectreApi from '../services/api';

const SSLRenewal = () => {
  return (
    <ScanResultPage
      title="SSL Renewal Tracker"
      description="Track SSL certificate renewal timeline"
      icon={Lock}
      fetchFunction={spectreApi.checkSSLRenewal}
      renderData={(data) => (
        <div className="scan-card">
          <h2 className="text-xl font-bold text-white mb-4">SSL Renewal Status</h2>
          <pre className="text-sm text-gray-300 overflow-auto bg-space-black/50 p-4 rounded-lg">
            {JSON.stringify(data, null, 2)}
          </pre>
        </div>
      )}
    />
  );
};

export default SSLRenewal;
